...
* The replicators declare their *current* availability to handle papers in the corresponding section of the `SS`, 

* by modifying column `Availab.` 

* `Availab. = 2` would mean that the replicator can handle 2 packages *starting today*: 

![Replicator Availability](images/step3.png)
...
1. Replicator navigates to shared `DB` folder `2. Submitted replication packages`, and looks for the correct submission number and Author names.
1. Downloads a **copy** of `3. Replication Package` to their local drive.
2. initiates a `git` repository on local machine to track code.

```bash
git init 
git add .  # the `.` adds everything in current dir
git commit -m 'initial check-in of code'
```
...
> tools::md5sum("3 Replication package.zip")
